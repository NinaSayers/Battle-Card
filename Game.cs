using BattleCardsEngine;
 #region Game
                            ...
                            while (!endPhase || !endTurn)
                            {
                                ...
                                if (this.rules.IsValidMove(jugada, phase, player, this.state))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            if (attacksCount == state.Table.GetMonsterCardsInvokeds(player.ID).Count) endPhase = true;
                                            else attacksCount += 1;
                                        }
    ...
    public class Actions /*: IAction*/ //falta agregar las acciones "terminar turno" y "terminar fase"
    {
        // // QUITAR ESTO
        ...
        public State Draw(int playerID, int gameID, State state)
        {
            ...
            MyExceptions.EmptyDeckException(state.Table.GetDeck(playerID));
            int countDeck = state.Table.GetDeck(playerID).Count;
            Card newcard = state.Table.GetDeck(playerID)[countDeck - 1];
            State newState = new State(state.GameTurns, state.TurnsByPlayer, state.ActualPhase, state.Players, state.Hands, state.Table, this.gameID, state.LifePoints);
        ...
        public State AttackCard(int gameID, int attackantPlayerID, MonsterCard cardAtacante, MonsterCard cardAtacada, State state)
        {
            ...
            MyExceptions.InvalidAttackantCardException(state.Table.GetMonsterCardsInvokeds(attackantPlayerID), cardAtacante);
            bool yaAtacó = false;
            State newState = new State(state.GameTurns, state.TurnsByPlayer, state.ActualPhase, state.Players, state.Hands, state.Table, this.gameID, state.LifePoints);
        ...
        public State AttackLifePoints(int gameID, int playerID, MonsterCard attackantCard, State state)
        {
            ...
            MyExceptions.InvalidAttackantCardException(state.Table.MonsterCardsInvokeds[playerID], attackantCard);
            State newState = new State(state.GameTurns, state.TurnsByPlayer, state.ActualPhase, state.Players, state.Hands, state.Table, this.gameID, state.LifePoints);
            newState.SetLifePoints(gameID, playerID, state.GetLifePoints(playerID) - attackantCard.Attack);
            return newState;
        }
    ...
    public struct State
    {
        ...
        public State(int gameTurns, List<int> turnsByPlayer, PhasesEnum actualPhase, List<Player> players, List<List<Card>> hands, Table table, int gameID, List<float> lifePoints)
        {
